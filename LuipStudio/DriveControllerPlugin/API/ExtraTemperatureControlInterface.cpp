 ...
 */
#include "Log.h"
#include "Code/ExtraTemperatureControlInterface.h"
...
#include "ExtraTemperatureControlInterface.h"
 ...
 * @param[in] addr 设备地址。
 */
ExtraTemperatureControlInterface::ExtraTemperatureControlInterface(DscpAddress addr)
...
TempCalibrateFactor ExtraTemperatureControlInterface::GetCalibrateFactor()
{
    TempCalibrateFactor factor = {0, 0, 0};

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_ETCI_GET_CALIBRATE_FACTOR));
 ...
 *  - @ref DSCP_ERROR 操作失败；
 */
bool ExtraTemperatureControlInterface::SetCalibrateFactor(TempCalibrateFactor factor)
{
    Uint16 status = DscpStatus::Error;

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_ETCI_SET_CALIBRATE_FACTOR, &factor, sizeof(factor)));
    SyncCaller syncCaller(m_retries);
 ...
 */
Temperature ExtraTemperatureControlInterface::GetTemperature()
{
    Temperature temp;
    memset(&temp, 0, sizeof(temp));

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_ETCI_GET_TEMPERATURE));
    SyncCaller  syncCaller(m_retries);
    DscpRespPtr resp = syncCaller.Send(cmd);
    if (resp)
    {
 ...
 */
ThermostatParam ExtraTemperatureControlInterface::GetThermostatParam()
{
    ThermostatParam param;
    memset(&param, 0, sizeof(param));

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_ETCI_GET_THERMOSTAT_PARAM));
    SyncCaller  syncCaller(m_retries);
    DscpRespPtr resp = syncCaller.Send(cmd);
    if (resp)
    {
...
* @return 返回状态，操作是否成功。
 */
bool ExtraTemperatureControlInterface::SetThermostatParam(ThermostatParam param)
{
    Uint16 status = DscpStatus::Error;

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_ETCI_SET_THERMOSTAT_PARAM, &param, sizeof(param)));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
    return (status == DscpStatus::OK);
}
...
Uint16 ExtraTemperatureControlInterface::GetThermostatStatus()
{
    Uint16 status = DscpStatus::Busy;

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_ETCI_GET_THERMOSTAT_STATUS));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
    return status;
}
 ...
 */
bool ExtraTemperatureControlInterface::StartThermostat(ThermostatMode mode, float targetTemp, float toleranceTemp, float timeout)
{
    Uint16 status = DscpStatus::Error;
    Uint8 cmdData[13] = {0};
 ...
 * @note 该命令将立即返回。
 */
bool ExtraTemperatureControlInterface::StopThermostat()
{
    Uint16 status = DscpStatus::Error;

    DscpCmdPtr cmd(new DscpCommand(m_addr, DSCP_CMD_ETCI_STOP_THERMOSTAT));
    SyncCaller syncCaller(m_retries);
    status = syncCaller.SendWithStatus(cmd);
 ...
 */
bool ExtraTemperatureControlInterface::SetTemperatureNotifyPeriod(float period)
{
    Uint16 status = DscpStatus::Error;
 ...
 *  - @ref temp float，当前温度。
 */
ThermostatResult ExtraTemperatureControlInterface::ExpectThermostat(long timeout)
{
 ...
 */
void ExtraTemperatureControlInterface::RegisterTemperatureNotice(IEventReceivable *handle)
{
    EventHandler::Instance()->Register(m_addr,DSCP_EVENT_ETCI_TEMPERATURE_NOTICE,handle);
}
