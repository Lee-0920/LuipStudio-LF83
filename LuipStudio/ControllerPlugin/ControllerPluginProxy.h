#ifndef CONTROLLERPLUGINPROXY_H
#define CONTROLLERPLUGINPROXY_H

#include "oolua.h"
#include "LuipShare.h"
#include "API/Version.h"
#include "API/ManufDate.h"
#include "API/DeviceStatusInterface.h"
#include "API/UpdatableInfo.h"
#include "API/DeviceInfoInterface.h"
#include "API/DeviceInterface.h"
 ...
 * @details
 */
OOLUA_PROXY(LUIP_SHARE,Version)
    OOLUA_CTORS(
        OOLUA_CTOR(int, int)
 ...
 * @details
 */
OOLUA_PROXY(LUIP_SHARE,ManufDate)
    OOLUA_CTORS(
        OOLUA_CTOR(int, int, int)
        OOLUA_CTOR(Byte*)
    )
 ...
 * @details
 */
OOLUA_PROXY(LUIP_SHARE,RunMode)
    OOLUA_TAGS(
 ...
 * @details
 */
OOLUA_PROXY(LUIP_SHARE,PowerSupplyType)
    OOLUA_TAGS(
        Register_class_enums  // 枚举需要特别标注，否则枚举值无效
    )
    OOLUA_ENUMS(
 ...
 */
OOLUA_PROXY(LUIP_SHARE, UpdatableInfo)
    OOLUA_MFUNC(Update)
 ...
 */
OOLUA_PROXY(LUIP_SHARE,ControllerInfo, UpdatableInfo)
...
OOLUA_PROXY(LUIP_SHARE,DeviceInterface)
    OOLUA_CTORS(
...
//        OOLUA_CTOR(DscpAddress, UpdatableInfo*)
    ...
    OOLUA_MFUNC(GetTimeout)
    OOLUA_MFUNC(SetTimeout)
    OOLUA_MFUNC(GetRetries)
    OOLUA_MFUNC(SetRetries)
OOLUA_PROXY_END
 ...
 */
OOLUA_PROXY(LUIP_SHARE,DeviceInfoInterface, DeviceInterface)
    OOLUA_TAGS(
 ...
 * @details 观察者。
 */
OOLUA_PROXY(LUIP_SHARE,IEventReceivable)
    // 特殊标记定义
    OOLUA_TAGS(
 ...
 */
OOLUA_PROXY(LUIP_SHARE,DeviceStatusInterface, DeviceInterface)
    OOLUA_TAGS(
        No_default_constructor //没有默认构造函数时可以使用此标签
    )
    OOLUA_CTORS(
    ...
    OOLUA_MFUNC(Reset)
//    OOLUA_MEM_FUNC(void, RegisterResetNotice, cpp_in_p<IEventReceivable*>)
    OOLUA_MFUNC(RegisterResetNotice)
 ...
 */
OOLUA_PROXY(LUIP_SHARE,DeviceUpdateInterface, DeviceInterface)
    OOLUA_TAGS(
        No_default_constructor //没有默认构造函数时可以使用此标签
    )
    OOLUA_CTORS(
 ...
 */
OOLUA_PROXY(LUIP_SHARE,DscpSystemInterface, DeviceInterface)
    OOLUA_TAGS(
        No_default_constructor //没有默认构造函数时可以使用此标签
    )
    OOLUA_CTORS(
 ...
 */
OOLUA_PROXY(LUIP_SHARE,BaseController, IEventReceivable)
    OOLUA_TAGS(
        No_default_constructor //没有默认构造函数时可以使用此标签
    )
    OOLUA_CTORS(
 ...
 * @details 观察者。
 */
OOLUA_PROXY(LUIP_SHARE,ISignalNotifiable)
    // 特殊标记定义
    OOLUA_TAGS(
        Abstract  // 指定抽象标记，oolua 不会生成 new()，且会忽略 OOLUA_CTORS()
    )
...
{
public:
    ControllerPluginProxy();

    static void Proxy();
};

#endif // CONTROLLERPLUGINPROXY_H
